...
enum { Stride = 2 };//alignof(UE_FNameEntry) = 2bit
...
class UE_UObject
{
    uintptr_t object;
public:
    UE_UObject(uintptr_t obj = 0) : object(obj) {}
    ...
    uintptr_t GetAddress() const { return object; }
...
class UE_FUObjectArray
{
    UE_UObject* Objects;
public:
    int32_t GetNumElements() const;
    UE_UObject GetObjectPtr(int32_t index) const;
    ...
    T FindObject(const std::string &fullName) const
    {
        for (int32_t i = 0; i < GetNumElements(); i++)
        {
            ...
            if (object.GetAddress() && object.GetFullName() == fullName)
            {
...
public:
    UE_FFieldClass(uintptr_t object) : object(object) {};
    UE_FFieldClass() : object(0) {};
    bool operator==(const UE_FFieldClass obj) const { return obj.object == object; };
    bool operator!=(const UE_FFieldClass obj) const { return obj.object != object; };
    uintptr_t GetAddress() const { return object; }
    operator uintptr_t() const { return object; };
...
public:
    UE_FField(uintptr_t object) : object(object) {}
    UE_FField() : object(0) {}
...
public:
    using UE_UField::UE_UField;
    TArray<uintptr_t> GetNames() const;//UE_TArray<TPair<UE_FName, uint64_t>>
    std::string GetName() const;
    static UE_UClass StaticClass();
};
...
bool UE_UObject::IsA() const{
    ...
    for (auto super = this->GetClass(); super.GetAddress(); super = super.GetSuper().Cast<UE_UClass>())
    {
        if (super == cmp)
        {
            return true;
